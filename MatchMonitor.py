...
def message_text(match_opponent, offences=0):
                ...
                try:
                    ...
                    if result == 'timeout' or result == 'resigned':
                        ...
                        if result == 'timeout' or resigned_early:
                            ...
                            year_df = df[df['date'] >= twelve_months_ago]
                            strike = year_df.value_counts(subset=['username', 'match']).get(username, pd.Series(dtype=float)).count()
                            ...
                            # Write message here
                            # message = message_text(match_opponent, strike)
                except KeyError:
...
# TODO: fix strike count (it's counting the previous game as a strike)
